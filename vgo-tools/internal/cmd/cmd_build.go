...
const (
	...
	cBuildEg    = `
vgo-tools build main.go
vgo-tools build main.go --pack public,template
vgo-tools build main.go --cgo
vgo-tools build main.go -m none 
vgo-tools build main.go -n my-app -a all -s all
vgo-tools build main.go -n my-app -a amd64,386 -s linux -p .
vgo-tools build main.go -n my-app -v 1.0 -a amd64,386 -s linux,windows,darwin -p ./docker/bin
`
	cBuildDc = `
