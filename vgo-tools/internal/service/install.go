...
func (s serviceInstall) Run(ctx context.Context) (err error) {
	...
	if len(paths) <= 0 {
		mlog.Printf("no path detected, you can manually install vgo-tools by copying the binary to path folder.")
		return
	}
	...
	if err != nil {
		mlog.Printf("install vgo-tools binary to '%s' failed: %v", dstPath.dirPath, err)
		mlog.Printf("you can manually install vgo-tools by copying the binary to folder: %s", dstPath.dirPath)
	} else {
		mlog.Printf("vgo-tools binary is successfully installed to: %s", dstPath.dirPath)
	}
...
func (s serviceInstall) getAvailablePaths() []serviceInstallAvailablePath {
	var (
		...
		binaryFileName = "vgo-tools" + gfile.Ext(gfile.SelfPath())
	)
	// $GOPATH/bin
