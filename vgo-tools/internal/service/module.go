...
func CreatModule(ctx context.Context, moduleName string) (err error) {
	...
	// 创建模块目录结构
	err = gres.Export("vgo-simple/modules/demo", moduleDir, gres.ExportOption{
		RemovePrefix: "vgo-simple/modules/demo",
	})
	if err != nil {
		return
	}
	// 替换import路径
	err = gfile.ReplaceDir("vgo-simple/modules/demo", module+"/modules/"+moduleName, moduleDir, "*", true)
	if err != nil {
		return
	}
